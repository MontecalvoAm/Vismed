                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                    <div className="space-y-3">
                        {items.filter((_, index) => initialItemIndex == null || initialItemIndex === index).map((item, localIndex) => {
                            ...
                            const max = item.Quantity;
                            // Check if this is a pharmacy item with qty <= 1, fully used, or one-time visit
                            const isOneTimeVisit = quotation.SessionType === 'One-time';
                            const isCompleted = isItemTrackingCompleted(item, isOneTimeVisit);
